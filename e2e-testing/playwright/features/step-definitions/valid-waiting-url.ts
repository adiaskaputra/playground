...
Then('I should see the button OpenPaceUpApp', { timeout: 20000 }, async function () {
  openAppButton = page.locator('button:has-text("OpenPaceUpApp")');
  await expect(openAppButton).toBeVisible({ timeout: 10000 });
});

When('I click the button OpenPaceUpApp', async () => {
  if (!openAppButton) {
    openAppButton = page.locator('button:has-text("OpenPaceUpApp")');
  }
