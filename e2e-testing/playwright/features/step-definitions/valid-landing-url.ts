...
Then('I should see "Download and Confirm your application viaPaceUpapp" in the title', { timeout: 20000 }, async function () {
  const title = page.locator('.content__title');
  await expect(title).toBeVisible({ timeout: 10000 });
  await expect(title).toHaveText('Download and Confirm your application viaPaceUpapp');
});
...
Then('I should see the QR code with valid deeplink', { timeout: 20000 }, async function () {
  ...
  expect(qrData).toMatch(/^https:\/\/landing\.test-app\.link\/[a-zA-Z0-9]+$/);
});
